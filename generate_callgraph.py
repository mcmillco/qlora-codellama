...
top_p = 1
temperature = 1e-9
f = open('data/callgraph_java_test.json')
srcml_filename = "predictions/callgraph_java_codellama.pkl"
...
adapter_path = 'callgraph_java_pretrained/checkpoint-705' # '/output' is not accepted
...
ori_data = pickle.load(open("/scratch/chiayi/llm_reason/callgraph/java/callgraph_extract_test.pkl", "rb"))
allcode = []

for data in ori_data:
    code = data["code"]
    method_name = data["method_name"]
    allcode.append((code, method_name))
...
for data in tqdm.tqdm(testdata[:]):
    ...
    ret = ret.split("<\s>")[0]
    ori_data[count]["result"] = ret.strip()
    if(count % 20 == 0):
